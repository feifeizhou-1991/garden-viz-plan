...
export const GardenGrid: React.FC<GardenGridProps> = ({
      ...
      for (let x = 0; x < width; x++) {
        ...
        cells.push(
          <div
            key={`${x}-${y}`}
            className={cn(
              "w-16 h-16 border-[6px] border-transparent bg-card/80 cursor-pointer transition-all duration-200 flex items-center justify-center text-3xl relative rounded-lg",
          ...
          >
            ...
            {!plantedCell && isHovered && !selectMode && (
              <Plus
                className="w-8 h-8 text-foreground pointer-events-none transition-opacity"
                strokeWidth={2.5}
              />
            )}
            {isSelected && (
  ...
  return (
    <div className="relative inline-block">
      <div
        className="inline-grid gap-2 p-8 rounded-xl border-border shadow-lg border-0 bg-garden-soil"
        style={{
      ...
      {mergedRegions.map((r) => {
        ...
        return (
          <div
            ...
            className="absolute pointer-events-none flex items-center justify-center bg-grid-merged border-[6px] border-grid-merged rounded-lg"
            style={{
              left: `${left}px`,
              top: `${top}px`,
              width: `${widthPx}px`,
